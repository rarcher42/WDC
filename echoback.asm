...
START 	SEI
		...
		JSR		PUTCHAR
ECHO_LOOP	
		JSR		GETCHAR				; attempt to get next character in	
...
LOAD_IT		
		RTS							; Add LOAD support here
SAVE_IT		
		RTS							; Add SAVE support here
...
INIT_FIFO
		...
		STZ 	SYSTEM_VIA_ACR		; Disable PB7, shift register, timer T1 interrupt.  Not absolutely
									; required while interrupts are disabled FIXME: set up timer
		STZ		SYSTEM_VIA_DDRA		; Set PA0-PA7 to all inputs
		STZ		SYSTEM_VIA_DDRB		; In case we're not coming off a reset, make PORT B an input 
									; and change output register when it's NOT outputting
		LDA		#FIFO_RD		
		STA		SYSTEM_VIA_IORB		; Avoid possible glitch by writing to output latch while Port B is still 
...
OFCONT		
		...
		STA		SYSTEM_VIA_IORA		; Set up output value in advance in Port A (still input so doesn't 
									; go out yet) 
		LDA		#$FF				; make Port A all outputs with stable output value already set 
									; in prior lines
